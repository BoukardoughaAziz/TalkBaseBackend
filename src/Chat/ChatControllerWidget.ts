...
@Controller('api/chat')
export class ChatControllerWidget {
  private readonly logger = new Logger(ChatControllerWidget.name);
  constructor(
  ...
  async initClientToCallCenterChatConversations(
    ...
    const conversations = await this.chatServiceClient.getConversations(
      chatMessage.appClient.identifier,
    );
  ...
  async getConversations(
    @Query('callCenterAgentId') callCenterAgentId: String,
  ) {
    let array: Array<ChatMessage> = new Array();
    const chatConversations =
      await this.chatServiceClient.getConversations(callCenterAgentId);
