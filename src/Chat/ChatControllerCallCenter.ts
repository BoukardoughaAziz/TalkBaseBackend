...
export class ChatControllerCallCenter {
  private readonly logger = new Logger(ChatControllerCallCenter.name);
  constructor(
  ...
  async   getCallCenterDashboard(
   ...
   @Post('/addMessageFromAgentToClient')
    async addMessageFromAgentToClient(@Body() incomingChatMessage: any) {
      ...
      this.logger.log(`Send message from Agent to Client : ${chatMessage}`);
    }
