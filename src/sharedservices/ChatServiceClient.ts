...
*/
   ...
   // try {
    ...
    const chatMessage= await  SharedServicesUtil.saveChatMessage(this.chatMessageModel,appClient,incomingChatMessage)
  ...
  async getConversations(clientIdentifier) {
    const chatMessages = await this.chatMessageModel
      .find({ 'appClient.identifier': clientIdentifier ,'chatEvent': { "$ne": "NewClientStartOpenChatWidget" }})
